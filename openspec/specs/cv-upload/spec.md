...
## Purpose
Specifies requirements for uploading and parsing CV documents to pre-fill user profile fields.
...
#### Scenario: User views the loaded CV
- **WHEN** a CV has been uploaded and the user clicks on the "Voir" button next to the CV status indicator
- **THEN** the system SHALL generate a temporary signed URL from Supabase Storage and open the file in a new browser tab
